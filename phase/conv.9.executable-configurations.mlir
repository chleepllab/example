#config = #iree_cpu.lowering_config<distribution = [32, 32, 32, 32, 0, 0, 0], vector_common_parallel = [1, 1, 1, 8, 0, 0, 0], vector_reduction = [0, 0, 0, 0, 1, 1, 16]>
...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  hal.executable private @conv_dynamic_dispatch_0 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @conv_dynamic_dispatch_0_conv_DxDxDxDxDxDxD_f32() attributes {translation_info = #translation} {
          ...
          %103 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%100, %101 : tensor<?x?x?x?xf32>, tensor<?x?x?x?xf32>) outs(%102 : tensor<?x?x?x?xf32>) attrs =  {lowering_config = #config} {
