hal.executable public @matmul_dynamic_dispatch_2 {
  hal.executable.variant public @embedded_elf_riscv_64 target(<"llvm-cpu", "embedded-elf-riscv_64", {cpu = "", cpu_features = "+m,+a,+f,+d,+zvl256b,+v", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_layout<>, max_stack_allocation_size = 32768 : i64, native_vector_size = 64 : i64, target_abi = "lp64d", target_triple = "riscv64-unknown-unknown-eabi-elf", ukernels = "all"}>) {
    ...
    builtin.module {
      func.func @matmul_dynamic_dispatch_2_pack_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDataTiling>} {
        ...
        %38 = iree_tensor_ext.dispatch.workload.ordinal %29, 2 : index
        %39 = iree_tensor_ext.dispatch.workload.ordinal %34, 3 : index
        %40 = hal.interface.binding.subspan layout(<constants = 10, bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(0) alignment(64) offset(%c0) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?xf32>>{%36, %37}
        %41 = hal.interface.binding.subspan layout(<constants = 10, bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(1) alignment(64) offset(%14) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x?x7x32xf32>>{%38, %39}
        %42 = iree_tensor_ext.dispatch.tensor.load %40, offsets = [0, 0], sizes = [%36, %37], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?xf32>>{%36, %37} -> tensor<?x?xf32>
        %43 = tensor.empty(%38, %39) : tensor<?x?x7x32xf32>
        %pack = linalg.pack %42 padding_value(%cst : f32) outer_dims_perm = [0, 1] inner_dims_pos = [0, 1] inner_tiles = [7, 32] into %43 {lowering_config = #iree_codegen.lowering_config<tile_sizes = [[9, 2], [1, 1]]>} : tensor<?x?xf32> -> tensor<?x?x7x32xf32>
        iree_tensor_ext.dispatch.tensor.store %pack, %41, offsets = [0, 0, 0, 0], sizes = [%38, %39, 7, 32], strides = [1, 1, 1, 1] : tensor<?x?x7x32xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x?x7x32xf32>>{%38, %39}
        return
      }
    }
  }
