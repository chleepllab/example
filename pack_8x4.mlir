func.func @matmul_dynamic(%lhs: tensor<?x?xf32>) -> tensor<?x?x8x4xf32> attributes { iree.compiler.export } {
  ...
  %dim1_div = arith.ceildivsi %dim1, %c4 : index
  %init = tensor.empty(%dim0_div, %dim1_div) : tensor<?x?x8x4xf32>
  %result = linalg.pack %lhs inner_dims_pos = [0, 1] inner_tiles = [8, 4] into %init
